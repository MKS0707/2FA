...
public class EncryptUtil {
	...
	private static byte[] decrypt(byte[] cipherText, byte[] key, byte[] initialVector) throws Exception {
	  ...
	  String data= "{'channel_UserId' : 'ddfd','channel_ID' : 7165}"; JSONObject
	  newObj = new JSONObject(data);
	  
	  String encryptedData = EncryptUtil.encryptString(newObj.toString(), "");
	  
	  JSONObject lastObj = new JSONObject(); lastObj.put("encryptedData",
	  encryptedData);
	  
	  System.out.println(lastObj.toString());
	  
	  String decryptedData = EncryptUtil.decryptString("DwQrsTeQzNIDSoHu7iVSnW7PBbip9ZNhVmZ0Tp4652rmumYvGGAgS27URLf4V4mcl3fcFD/oP9ZYPl150BnOkg==", "");
